 ...
 * Copyright (c) 2025, Qualcomm Innovation Center, Inc. All rights resereted.
 *
...
int ipq4019_mdio_read(struct udevice *dev, int addr, int devad, int reg)
{
	...
	unsigned int ctrl_0, cmd;

	if (ipq4019_mdio_wait_busy(priv))
		return -ETIMEDOUT;

	ctrl_0 = readl(priv->mdio_base + MDIO_MODE_REG);
	ctrl_0 &= ~MDIO_CLK_DIV_MASK;
	ctrl_0 |= FIELD_PREP(MDIO_CLK_DIV_MASK, priv->clk_div);

	if (devad != MDIO_DEVAD_NONE) {
		/* Enter Clause 45 mode */
		ctrl_0 |= MDIO_MODE_BIT;
		writel(ctrl_0, priv->mdio_base + MDIO_MODE_REG);
...
static int ipq4019_mdio_probe(struct udevice *dev)
{
	...
	if (clk_div != 0) {
		if (((clk_div != 0xFF) && ((clk_div + 1) & clk_div)) != 0) {
		/*
		 * supported div factor 8, 16, 32, 64, 128 & 256
		 * div factor = div + 1
		 * default frequency is 6.25MHz
		 */
			dev_dbg(dev, "clk div factor not in range, "
					"use default (0x0F)\n");
			clk_div = 0x0F;
		}
	}
